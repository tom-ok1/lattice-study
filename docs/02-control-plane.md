...
hold 中の追加変更は dirty 状態へまとめ、すでに設定した期限を後ろへずらさない。これにより変更が続いても SPF が無期限に先送りされない。
timer は generation を持ち、再スケジュール前の古い timer は無視する。SPF 1 回につき経路表の差し替えは最大 1 回とし、計算結果が同じなら公開しない。
