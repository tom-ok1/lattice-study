...
設計時の推奨順から意図的に変更し、現在は **Phase 2 必須コア → Phase 4 live Pub/Sub** の順で進める。

- Phase 2：単一最短経路、QoS、Explicit Multicast、runtime/TCP 接続までを完了。ECMP、診断 CLI、
  Prometheus、`tc netem` は後続バックログ
- Phase 3：未完了のまま意図的に延期。完了扱いでも中止でもない
- Phase 4：LSA ベースの subscriber discovery まで完了。アプリ向け gRPC API 以降は未実装

Phase 3 を先送りするため、50 ノード churn やシード再現による検証はまだ行えない。この技術的負債を限定するため、
core crate に I/O・実時刻を入れないこと、決定的なコレクションを使うこと、状態機械テストと実 TCP 統合テストを
併用することを、保留期間中も必須条件とする。
...
**現在の状態：未完了・意図的に延期。Phase 4 の縦切りを先に進めている。**
...
> 元の推奨順では Phase 3 を Phase 4 より先に置く。今回は学習対象として live Pub/Sub を優先するため順序を変えるが、
> 後付け時の大工事を避けるため、Pub/Subも引き続きI/Oなしの`Component`境界を維持する。
